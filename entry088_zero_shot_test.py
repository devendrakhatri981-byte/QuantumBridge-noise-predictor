...
RECORDS_PATH = "quantumbridge_data/entry087_cusco_bell_dataset.json"
GRAPHS_PATH = "quantumbridge_data/entry088_cusco_graph_dataset.json"
...
def build_graphs():
    ...
    print(f"resuming with {done}/{len(records)} cusco eval graphs already built")

    backend = BACKENDS["cusco"]()
    graph = em.build_connectivity_graph(em.load_calibration("cusco"), "cusco")
    coh = v4.load_coherence("cusco")

    for i, r in enumerate(records):
        if i < done:
            continue
        g = graph_for_record(backend, graph, coh, r)
        g.update({"chip": "cusco", "kind": r["kind"],
    ...
    json.dump(out, open(tmp, "w"))
    os.replace(tmp, GRAPHS_PATH)
    print(f"DONE: {len(out)} cusco eval graphs -> {GRAPHS_PATH}")
    return out
...
if __name__ == "__main__":
    ...
    # Cusco has NO entry in node_stats/edge_stats (never trained on -- it is the new clean holdout) -- use
    ...
    print(f"\n=== TRUE ZERO-SHOT: Entry 085 5-chip model (never saw Cusco) on {len(graphs)} Cusco circuits ===")
    ...
    result = {"chip": "cusco", "n": len(graphs), "mae": float(mae), "r2": float(r2),
             "mean_std": float(y_std.mean()), "mae_fc": float(mae_fc) if mae_fc is not None else None,
             "n_fc": int(is_fc.sum())}
    tmp = "quantumbridge_data/entry088_zero_shot_results.json.tmp"
    json.dump(result, open(tmp, "w"), indent=2)
    import os
    os.replace(tmp, "quantumbridge_data/entry088_zero_shot_results.json")
    print("saved -> quantumbridge_data/entry088_zero_shot_results.json")
