"""
QuantumBridge — Entry 055: GNN capacity bump #2, 48/64 -> 80/104.
...
42 nodes/56 edges) to 80/104 (fits Entry 055's max: 65 nodes/99 edges,
...
MAX_N, MAX_E = 80, 104
IN_PATH = "quantumbridge_data/entry055_graph_dataset.json"
...
FOLD_CKPT = "quantumbridge_data/entry055_gnn_folds.json"
RESULTS_PATH = "quantumbridge_data/entry055_gnn_results.json"
EPOCHS = 100


def train_fold(train_idx, test_idx, seed=0, epochs=EPOCHS, batch_size=64, lr=3e-3, chunk_epochs=25):
    import os
    ckpt_path = f"quantumbridge_data/entry055_fold_tmp_ckpt.json"
    opt = optax.adam(lr)
