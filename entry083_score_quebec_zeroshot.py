"""QuantumBridge — Entry 083: score all 8,001 Quebec pairs with the Entry 077
deployed model, using the SAME true zero-shot method as Entry 078 (averaged
4-training-chip normalization stats, since Quebec was never trained on).
Unlike Entry 078's 710-circuit sample, this covers every possible pair, so
the live demo can offer Quebec as a "pick any two qubits, see genuine
zero-shot generalization" showcase rather than a fixed sample.

Merges into entry081_lookup_with_uncertainty.json under a "quebec" key,
alongside the four trained chips -- but the demo must treat this key
differently (always show a zero-shot/cold badge, never a "cached" badge
implying trained-chip confidence)."""
...
model = json.load(open("quantumbridge_data/entry077_deploy_params.json"))
...
TRAIN_CHIPS = ("kyiv", "sherbrooke", "brisbane", "osaka")
node_mu_avg = np.mean([node_stats[c][0] for c in TRAIN_CHIPS], axis=0)
node_sd_avg = np.mean([node_stats[c][1] for c in TRAIN_CHIPS], axis=0)
edge_mu_avg = np.mean([edge_stats[c][0] for c in TRAIN_CHIPS], axis=0)
edge_sd_avg = np.mean([edge_stats[c][1] for c in TRAIN_CHIPS], axis=0)
...
def forward_stochastic(node_feat, node_mask, src, dst, edge_feat, edge_mask, glob, rng):
    nf = (node_feat - node_mu_avg) / node_sd_avg
    ef = (edge_feat - edge_mu_avg) / edge_sd_avg
...
def predict_mc(g, rng):
    padded = pad_graph(g)
    samples = [forward_stochastic(*padded, rng) for _ in range(T_SAMPLES)]
...
def ground_truth_index():
    idx = {}
    for path in ("quantumbridge_data/entry076_quebec_bell_dataset.json",):
        try:
        ...
        for r in recs:
            if r["kind"] != "bell":
                continue
            a, b = r["pairs_flat"]
            idx[(min(a, b), max(a, b))] = r["aer_ground_truth"]
    return idx


if __name__ == "__main__":
    all_pairs = json.load(open("quantumbridge_data/entry046_all_pairs.json"))["quebec"]
    gt = ground_truth_index()
    rng = np.random.default_rng(SEED)

    LOOKUP_PATH = "quantumbridge_data/entry081_lookup_with_uncertainty.json"
    out = json.load(open(LOOKUP_PATH))
    ...
    out["quebec"] = rows
    print(f"quebec: scored {len(rows)} pairs, "
          f"{sum(1 for r in rows if r[8] is not None)} have real ground truth (from the 710-circuit holdout sample)")
