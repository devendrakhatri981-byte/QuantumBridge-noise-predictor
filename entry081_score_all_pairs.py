"""
QuantumBridge — Entry 072: re-score all 24,003 pairs with the Entry 071
unified three-chip MC-Dropout model, producing mean +/- std (calibrated
uncertainty) for every pair on every chip -- replacing both the old
Entry 057 lookup (Kyiv/Sherbrooke only, 2-chip-era model) and the Entry
070 combined lookup (Brisbane, point-estimate only, no uncertainty).
Pure numpy, T=20 stochastic passes per pair, no Aer simulation needed.
"""
...
def ground_truth_index():
    ...
    for path in ("quantumbridge_data/entry043_combined_dataset.json",
                ...
                "quantumbridge_data/entry067_sherbrooke_bell_dataset.json",
                "quantumbridge_data/entry073_osaka_bell_dataset.json"):
        try:
...
if __name__ == "__main__":
    ...
    OUT_PATH = "quantumbridge_data/entry081_lookup_with_uncertainty.json"
    out = json.load(open(OUT_PATH)) if __import__("os").path.exists(OUT_PATH) else {}

    chips_to_do = sys.argv[1:] if len(sys.argv) > 1 else ["kyiv", "sherbrooke", "brisbane", "osaka"]
    for chip in chips_to_do:
        if chip in out:
            continue
        rows = []
    ...
    sz = __import__("os").path.getsize(OUT_PATH)
    print(f"saved lookup table: {sz/1e6:.2f} MB, chips done: {list(out.keys())}")
