"""QuantumBridge — Entry 077: fold Osaka's 708 eval-only bell-pair circuits
(grown in Entry 073 as a clean zero-shot holdout, now graduating into the
training pool now that Quebec has taken over the holdout role) into the
combined graph dataset (5,690 -> ~6,398, four chips: kyiv/sherbrooke/
brisbane/osaka)."""
...
NEW_RECORDS_PATH = "quantumbridge_data/entry073_osaka_bell_dataset.json"
BASE_GRAPHS_PATH = "quantumbridge_data/entry068_graph_dataset.json"
OUT_PATH = "quantumbridge_data/entry077_graph_dataset.json"


def main():
    base = json.load(open(BASE_GRAPHS_PATH))
    new_records = json.load(open(NEW_RECORDS_PATH))
    print(f"base graphs: {len(base)}, new osaka records: {len(new_records)}")
