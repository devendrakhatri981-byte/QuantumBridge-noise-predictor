"""
QuantumBridge — Entry 020: Find contrasting low/high error edges on
Sherbrooke, excluding placeholder 100% error entries (disabled/uncalibrated
qubit pairs, not genuine high-noise measurements).
"""
...
calibrated_real_edges = [(edge, err) for edge, err in error_lookup.items()
                          if edge in real_edge_set and err < 0.5]  # exclude placeholder 100% entries
calibrated_real_edges.sort(key=lambda x: x[1])

print(f"Total calibrated, real, PLAUSIBLE edges (excluding >=50% placeholders): {len(calibrated_real_edges)}")
excluded = len(error_lookup) - len(calibrated_real_edges)
print(f"Excluded as likely placeholders: {excluded}\n")
