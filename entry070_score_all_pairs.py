"""
QuantumBridge — Entry 070: re-score all 24,003 pairs with the Entry 069
combined three-chip model (retrained after Entry 067/068's Sherbrooke
growth balanced the dataset -- 5,690 graphs vs Entry 065/066's 4,886).

Also widens the ground-truth index to include the 804 new Sherbrooke
bell circuits from entry067_sherbrooke_bell_dataset.json, so pairs that
were only predictions before (no real simulated result) now show real
ground truth in the demo where we've actually simulated them.
"""
...
params_json = json.load(open("quantumbridge_data/entry069_combined_params.json"))
...
if __name__ == "__main__":
    ...
    tmp = "quantumbridge_data/entry070_combined_lookup.json.tmp"
    json.dump(out, open(tmp, "w"))
    import os
    os.replace(tmp, "quantumbridge_data/entry070_combined_lookup.json")
    sz = os.path.getsize("quantumbridge_data/entry070_combined_lookup.json")
