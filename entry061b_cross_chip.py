"""
QuantumBridge — Entry 061: the real three-chip generalization test.

Entries 047-060 established cross-chip generalization between Kyiv and
Sherbrooke, but that's still only one pair -- it's fair to ask whether it
was a genuine physical-noise-structure signal or a coincidence of those
two specific chips. This adds Brisbane (a third, independently-calibrated
127-qubit chip the model has never been trained on in any prior entry)
and runs the sharpest possible version of the test: train on the two
original chips combined, evaluate cold on the chip that contributed zero
training examples (kyiv_sherbrooke_to_brisbane) -- and the reverse,
train on Brisbane alone (a much smaller dataset, 752 circuits vs the
~3,200 Kyiv+Sherbrooke have), evaluate on Kyiv+Sherbrooke combined
(brisbane_to_kyiv_sherbrooke).
"""
...
import entry061b_train_gnn as base
...
def run(name, train_idx, test_idx, epochs=100, chunk_epochs=40, batch_size=64, lr=3e-3, seed=0):
    ckpt_path = f"quantumbridge_data/entry061b_cc_{name}_ckpt.json"
    opt = optax.adam(lr)
    ...
    result = {"name": name, "mae": float(mae), "r2": float(r2),
             ...
             "n_train": int(n_train), "n_test": len(test_idx), "n_fc": int(is_fc.sum())}
    print(f"[{name}] DONE: MAE={mae*100:.2f} R2={r2:.3f} "
          f"fc_MAE={(mae_fc*100 if mae_fc is not None else -1):.2f} (n_train={n_train}, n_fc={int(is_fc.sum())})")
    try:
        os.remove(ckpt_path)
...
if __name__ == "__main__":
    import sys
    direction = sys.argv[1] if len(sys.argv) > 1 else "kyiv_sherbrooke_to_brisbane"
    idx_all = np.arange(len(base.raw))
    if direction == "kyiv_sherbrooke_to_brisbane":
        train_idx = idx_all[(base.chip_all == "kyiv") | (base.chip_all == "sherbrooke")]
        test_idx = idx_all[base.chip_all == "brisbane"]
    else:
        train_idx = idx_all[base.chip_all == "brisbane"]
        test_idx = idx_all[(base.chip_all == "kyiv") | (base.chip_all == "sherbrooke")]

    RESULTS_PATH = "quantumbridge_data/entry061b_cross_chip_results.json"
