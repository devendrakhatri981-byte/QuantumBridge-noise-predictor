"""
QuantumBridge — Entry 053: GNN capacity bump #2, 48/64 -> 80/104.
...
42 nodes/56 edges) to 80/104 (fits Entry 053's max: 65 nodes/99 edges,
...
MAX_N, MAX_E = 80, 104
IN_PATH = "quantumbridge_data/entry053_graph_dataset.json"
...
FOLD_CKPT = "quantumbridge_data/entry053_gnn_folds_v2.json"
RESULTS_PATH = "quantumbridge_data/entry053_gnn_results.json"
EPOCHS = 100


def train_fold(train_idx, test_idx, seed=0, epochs=EPOCHS, batch_size=64, lr=3e-3, chunk_epochs=25):
    import os
    ckpt_path = f"quantumbridge_data/entry053_fold_tmp_ckpt_v2.json"
    opt = optax.adam(lr)
...
if __name__ == "__main__":
    ...
    for fold_i, (train_idx, test_idx) in enumerate(splits):
        if str(fold_i) in done:
            continue
        _, test_pred, target = train_fold(train_idx, test_idx, seed=fold_i)
