...
Exports FakeCusco's real topology and calibration data, following the same
pattern as setup_quebec.py (Entry 076). Cusco takes over Osaka's former role
...
with open("quantumbridge_data/real_topology_cusco.json", "w") as f:
    json.dump(undirected_edges, f, indent=2)
print("Saved to quantumbridge_data/real_topology_cusco.json")
...
calibration = {"chip": "cusco", "two_qubit_gate_errors": two_qubit_gate_errors}
with open("quantumbridge_data/offline_calibration_cusco_full.json", "w") as f:
    json.dump(calibration, f, indent=2)
print(f"Saved {len(two_qubit_gate_errors)} two-qubit gate error entries "
      f"to quantumbridge_data/offline_calibration_cusco_full.json")
