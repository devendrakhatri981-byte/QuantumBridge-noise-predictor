"""
QuantumBridge — Entry 076 Setup: Fifth Chip, the New Clean Zero-Shot Holdout

Exports FakeQuebec's real topology and calibration data, following the same
pattern as setup_osaka.py (Entry 073). Quebec takes over Osaka's former role
...
training to build the strongest possible combined model (Entry 077+).
"""
...
with open("quantumbridge_data/real_topology_quebec.json", "w") as f:
    json.dump(undirected_edges, f, indent=2)
print("Saved to quantumbridge_data/real_topology_quebec.json")
...
calibration = {"chip": "quebec", "two_qubit_gate_errors": two_qubit_gate_errors}
with open("quantumbridge_data/offline_calibration_quebec_full.json", "w") as f:
    json.dump(calibration, f, indent=2)
print(f"Saved {len(two_qubit_gate_errors)} two-qubit gate error entries "
      f"to quantumbridge_data/offline_calibration_quebec_full.json")
