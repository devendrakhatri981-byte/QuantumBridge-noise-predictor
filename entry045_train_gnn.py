"""
QuantumBridge — Entry 045c: GNN capacity bump.

Same architecture and feature set as Entry 044 (6 node feats, 4 edge
feats, 2 message-passing rounds, H=16) -- only MAX_N/MAX_E changed, from
40/40 (sized to fit the old dataset's largest circuit: 37 nodes) to
48/64 (sized to fit Entry 045's largest real circuit: 42 nodes / 56
edges, from the new 6-8 target star-GHZ circuits, with headroom for
future growth). Trained on the combined 2,317-circuit dataset (2,288
from Entry 044 + 29 new large-GHZ circuits from Entry 045a/b).

This directly tests whether the architecture holds up once it actually
has to represent a bigger slice of the chip per circuit, not just a
bigger padding constant.
"""
...
FOLD_CKPT = "quantumbridge_data/entry045_gnn_folds.json"
RESULTS_PATH = "quantumbridge_data/entry045_gnn_results.json"
EPOCHS = 180
...
if __name__ == "__main__":
    ...
    is_fc = np.array([abs(g["aer_ground_truth"] - 0.5) < 0.02 and g["bfs_hop_distance"] <= 10
                      for g in raw])
    is_big = np.array([g["n_nodes"] > 20 for g in raw])
    print(f"n={n}, floor-collapse n={is_fc.sum()}, big-circuit (>20 nodes) n={is_big.sum()}")
    ...
    if len(done) == 5:
        ...
        fold_mae_big = [done[str(i)]["mae_big"] for i in range(5) if done[str(i)]["mae_big"] is not None]

        print(f"\nGNN CV MAE={np.mean(fold_mae)*100:.2f} R2={np.mean(fold_r2):.3f} "
              f"floor-collapse MAE={np.mean(fold_mae_fc)*100:.2f} "
              f"big-circuit MAE={np.mean(fold_mae_big)*100:.2f}")

        v41_mae = mean_absolute_error(y_all, v41_all)
        v41_fc_mae = mean_absolute_error(y_all[is_fc], v41_all[is_fc])
        v41_big_mae = mean_absolute_error(y_all[is_big], v41_all[is_big])
        print(f"v4.1: MAE={v41_mae*100:.2f} floor-collapse MAE={v41_fc_mae*100:.2f} "
              f"big-circuit MAE={v41_big_mae*100:.2f}")

        result = {
            "n": n, "n_floor_collapse": int(is_fc.sum()), "n_big": int(is_big.sum()),
            ...
            "gnn_cv_mae_big": float(np.mean(fold_mae_big)),
            "v41_mae": float(v41_mae), "v41_mae_floor_collapse": float(v41_fc_mae),
            "v41_mae_big": float(v41_big_mae),
        }
