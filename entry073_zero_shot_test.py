...
RECORDS_PATH = "quantumbridge_data/entry073_osaka_bell_dataset.json"
GRAPHS_PATH = "quantumbridge_data/entry073_osaka_graph_dataset.json"
...
def build_graphs():
    ...
    print(f"resuming with {done}/{len(records)} osaka eval graphs already built")

    backend = BACKENDS["osaka"]()
    graph = em.build_connectivity_graph(em.load_calibration("osaka"), "osaka")
    coh = v4.load_coherence("osaka")

    for i, r in enumerate(records):
        if i < done:
            continue
        g = graph_for_record(backend, graph, coh, r)
        g.update({"chip": "osaka", "kind": r["kind"],
    ...
    json.dump(out, open(tmp, "w"))
    os.replace(tmp, GRAPHS_PATH)
    print(f"DONE: {len(out)} osaka eval graphs -> {GRAPHS_PATH}")
    return out
...
if __name__ == "__main__":
    ...
    model = json.load(open("quantumbridge_data/entry071_deploy_params.json"))
    ...
    # Osaka has NO entry in node_stats/edge_stats (never trained on) -- use
    ...
    node_mu_avg = np.mean([node_stats[c][0] for c in ("kyiv", "sherbrooke", "brisbane")], axis=0)
    node_sd_avg = np.mean([node_stats[c][1] for c in ("kyiv", "sherbrooke", "brisbane")], axis=0)
    edge_mu_avg = np.mean([edge_stats[c][0] for c in ("kyiv", "sherbrooke", "brisbane")], axis=0)
    edge_sd_avg = np.mean([edge_stats[c][1] for c in ("kyiv", "sherbrooke", "brisbane")], axis=0)
    ...
    print(f"\n=== TRUE ZERO-SHOT: Entry 071 model (never saw Osaka) on {len(graphs)} Osaka circuits ===")
    ...
    result = {"chip": "osaka", "n": len(graphs), "mae": float(mae), "r2": float(r2),
             "mean_std": float(y_std.mean()), "mae_fc": float(mae_fc) if mae_fc is not None else None,
             "n_fc": int(is_fc.sum())}
    tmp = "quantumbridge_data/entry073_zero_shot_results.json.tmp"
    json.dump(result, open(tmp, "w"), indent=2)
    import os
    os.replace(tmp, "quantumbridge_data/entry073_zero_shot_results.json")
    print("saved -> quantumbridge_data/entry073_zero_shot_results.json")
