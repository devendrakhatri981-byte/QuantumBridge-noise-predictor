"""
QuantumBridge — Entry 076: eval-only bell-pair dataset for Quebec, the true
fourth chip. Unlike Brisbane (Entry 061), this dataset is NEVER folded into
any training pipeline -- its only purpose is to give the deployed Entry 071
model (trained on Kyiv+Sherbrooke+Brisbane, zero Quebec exposure) a set of
real Aer/MPS ground-truth circuits to be scored against cold. This is the
genuine zero-shot generalization test this project has not yet run.

Identical methodology to entry061_grow_brisbane.py / entry067_grow_sherbrooke.py
(Aer/MPS ground truth, 4 seeds x 4096 shots folded into one call,
optimization_level=3, seed_transpiler=1, exact_dwell_cost prediction, same
six hop-distance bins, atomic checkpointed writes, shuffled task order).
"""
...
SHOTS = 4096
SEEDS = (1, 2, 3, 4)
OUT_PATH = "quantumbridge_data/entry076_quebec_bell_dataset.json"
BACKENDS = {"quebec": FakeQuebec}
...
def main(target=700, per_bin=None, seed=76, n_workers=4):
...
if __name__ == "__main__":
    import sys
    target = int(sys.argv[1]) if len(sys.argv) > 1 else 700
    main(target=target)
